...
class OcrPipeline(context: Context) : AutoCloseable {
    ...
    private fun cropQuad(img: Mat, box: Array<Point>): Mat? {
        val w = max(hypot(box[0].x - box[1].x, box[0].y - box[1].y),
                    hypot(box[2].x - box[3].x, box[2].y - box[3].y)).toInt()
        val h = max(hypot(box[0].x - box[3].x, box[0].y - box[3].y),
                    hypot(box[1].x - box[2].x, box[1].y - box[2].y)).toInt()
        if (w <= 1 || h <= 1) return null
        val src = MatOfPoint2f(*box)
        val dst = MatOfPoint2f(Point(0.0, 0.0), Point(w.toDouble(), 0.0),
