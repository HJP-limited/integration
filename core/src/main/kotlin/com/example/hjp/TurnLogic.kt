...
/**
 * 한 턴이 끝난 뒤 세션 상태를 갱신한다. **다음 턴의 재작성이 여기서 만든 값에 의존한다** —
 * focus 인물이 없으면 "그 사람 회사 어디야" 가 아무 사람이나 데려온다.
 *
 * 예전에는 이 블록이 앱 화면 코드 안에 있었다. 그래서 같은 :core 를 쓰는 데스크톱 러너가
 * 후속 발화를 재현하지 못했다 — 규칙은 공유하는데 그 규칙이 읽는 상태를 만드는 코드가
 * 앱에만 있었던 것이다(실측: 러너에서 "그 사람 회사 어디야" 가 엉뚱한 사람을 물어왔다).
 * 턴을 처리하는 쪽은 [runChat] 직후 반드시 이 함수를 불러야 한다.
 */
fun recordTurnState(session: AgentSession, question: String, result: ChatResult) {
