...
**러너는 생성 모델을 싣지 않는다.** Gemma 를 못 돌린다는 뜻이 아니다 — 에뮬레이터에 앱을
올리면 노트북에서도 Gemma 4 E2B 가 돌고 실제로 답변한다(x86_64 빌드 있음, 실측 턴당 ~80초).
러너에 안 넣은 것뿐이다. 그래서 `turn` 이 검증하는 범위는:

- **된다** — 기능/자기참조/전체개수 우회, 질의 재작성(담화참조·정정·속성이월·조건누적),
  검색·필드필터·기권. 생성이 없어 초 단위로 끝나므로 규칙을 고칠 때마다 돌릴 수 있다.
- **안 된다** — followup / context_answer 분기와 focus 인물 치환. `runChat` 이 이 분기를
  LLM 로드 **뒤에** 두어서 모델 없이는 지나가지 않는다.

그 층까지 노트북에서 보려면 에뮬레이터를 쓰거나, 러너에 데스크톱 LLM 런타임을 붙여야 한다.
에뮬레이터는 턴당 ~80초라 377턴 평가에는 8시간이 걸려 실용적이지 않다.
...
정리하면 "못 쓴다"가 아니라 **"확인하지 않았고, 바꿀 이유가 약하다"** 이다.

- Room 2.6.1 이 제공하는 애너테이션은 `@Fts3`, `@Fts4` 뿐이다(jar 확인). `@Fts5` 는 없다.
- raw SQL 로 `CREATE VIRTUAL TABLE ... USING fts5` 를 만들 수는 있으나, 기기의 플랫폼
  SQLite 가 FTS5 를 포함해야 한다. 이건 기기·API 레벨마다 갈리고 **우리는 실기기에서
  확인해 본 적이 없다.** 확실히 하려면 SQLite 를 직접 번들해야 한다(APK 증가).
- 무엇보다 지금 설정이 실측으로 고른 것이다: Recall@1 0.76→0.94, MRR 0.81→0.96,
  전화번호 조회 0.19→1.00. FTS5 로 바꾸면 이 지표를 전부 다시 재야 한다.
- FTS5 의 이점(bm25 랭킹, trigram 토크나이저로 한글 부분매칭)은 지금 4단 티어의
  LIKE 폴백이 상당 부분 맡고 있다.
